...
export default {
    ...
    methods: {
        ...
        /* collections */
        async getCollectionsByDate(user_id){
            try {
                if (user_id==0) {
                    await axios.get(`/api/v1/payments/?start_date=${this.start_date}&end_date=${this.end_date}`).then((response) => {
                        this.payments = response.data;
                        
                    });
                } else {
                    await axios.get(`/api/v1/payments/?start_date=${this.start_date}&end_date=${this.end_date}&user_id=${user_id}`).then((response) => {
                        this.payments = response.data;
                    });
                }
            } catch (error) {
                console.log('Error fetching payemtns by date', error);
            }
...
<template>
    <Layout>
        <pageheader title="Collection list" pageTitle="Collections" />
        <BRow>
                ...
                <BCard no-body class="table-card">
                    <BCardBody>
                        <div class="d-flex justify-content-between align-items-center p-sm-4 pb-sm-2">
                            ...
                            <div class="input-group input-group-sm search-bar mb-3">
                            ...
                            <div class="input-group input-group-sm search-bar mb-3">
                                ...
                                <flat-pickr v-model="date_start" :first-day-of-week="1" lang="en" confirm
                                        class="form-control form-control-sm" placeholder="Date Start"
                                        @on-change="getCollectionsByDate(authStore.user.id)"
                                ></flat-pickr>
                                <flat-pickr v-model="date_end" :first-day-of-week="1" lang="en" confirm
                                        class="form-control form-control-sm ms-1" placeholder="Date End"
                                        @on-change="getCollectionsByDate(authStore.user.id)">
                                </flat-pickr>
                            </div>
                            <!-- Add Subscriber Button -->
                                <!-- <router-link to="/add-product" class="btn btn-primary"> <i class="ti ti-plus f-18"></i> Add Region </router-link>-->
                            <BButton variant="primary" class="btn btn-sm mb-3" @click="showAddModalOpen">Add Collection</BButton>
                        </div>
